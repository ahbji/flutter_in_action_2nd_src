...
import 'package:sliver_persistent_header_to_box/extra_info_constraints.dart';
...
class SliverPersistentHeaderToBox extends StatelessWidget {
  ...
  }) : builder = ((context, maxExtent, fixed) => child);
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          return builder(
            ...
            // 通过约束传递的额外信息是一个bool类型，表示 Sliver 是否已经固定到顶部
  ...
  void performLayout() {
    ...
    child!.layout(
      ExtraInfoBoxConstraints(
        // 只要 constraints.scrollOffset 不为 0，即已经固定到顶部了
      ...
      // 我们要根据 child 大小来确定 Sliver 大小，所以后面需要用到 child 的大小（size）信息
      parentUsesSize: true,
    );
